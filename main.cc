...
void	tellOptions(const char *progname)
{
	...
	cout << "\t-K\t\tuse the MC Karel(Awesome)\n";
...
{
	char  options[] = "s:a:tvcrfFnNK"; // De opties die we willen herkennen
	//
	...
	do {
		...
		switch (opt) {	// welke optie is dit?
			...
			case 'K': // -b = BestFit allocator gevraagd
				require(beheerder == 0);
				beheerder = new McKarel(cflag);
				break;
				/*
...
int  main(int argc, char *argv[])
{
	...
	try {
		...
		check(size > 0);			// Omvang van het beheerde geheugen controleren

		beheerder->setSize(size);	// Vertel het aan de geheugen-beheerder

		Application  *mp = new Application(beheerder, size);	// en maak dan de pseudo-applicatie
